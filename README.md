...
> Production-grade data pipeline on a live SaaS product: Supabase to Snowflake, dbt, MetricFlow, and a LangGraph AI analyst.

Built on top of [OneOnOne](https://github.com/whysokara/oneonone), a shared work journal for managers and their teams. Real users, real subscriptions, real MRR.
...
Model-level DAG from source tables through staging, intermediate, and marts, generated by `dbt docs`:
...
| dbt staging | ✅ Shipped | 9 models, camelCase to snake_case, TIMESTAMP_TZ |
...
|---|---|---|
| `stg_users` | `users` | camelCase to snake_case, cast to TIMESTAMP_TZ |
| `stg_boards` | `boards` | camelCase to snake_case |
| `stg_memberships` | `memberships` | full-refresh |
| `stg_entries` | `entries` | camelCase to snake_case, entry lifecycle status |
| `stg_subscriptions` | `subscriptions` | snake_case, plan / billing_cycle / status |
| `stg_announcements` | `announcements` | camelCase to snake_case |
...
**Managers** (`sem_managers` on `dim_managers`)
...
**Entries** (`sem_entries` on `fct_entries`)
| Metric | Label | Description |
|---|---|---|
| `team_entries` | Team Entries | All team entries, excluding manager self-entries |
| `published_entries` | Published Entries | Published team entries (excludes drafts and self-entries) |
...
**Subscriptions** (`sem_subscriptions` on `fct_subscriptions`)
| Metric | Label | Description |
|---|---|---|
...
Business logic built into the data layer. Each rule exists because I built OneOnOne and know why it matters:
...
- **Annual MRR normalization:** `annual_price / 12`, not `monthly_price x 12`. Annual plans include 2 free months, so these are different numbers.
...
**Atomic ingestion:** raw tables load into `_STAGING` tables first, then swap in a transaction. dbt never sees a half-loaded RAW table.

**Pagination caught 46% data loss:** `entries` had 1,863 rows but Supabase's default cap silently returned 1,000. Found it with an independent `count='exact'` check, fixed with a `.range()` loop.

**Semantic layer before AI agent:** MetricFlow built in Week 4; the LangGraph agent (Week 5) calls `mf query` under the hood. The agent needs trusted metric definitions to cite, not raw SQL.
...
python ingestion/ingest.py        # Supabase to Snowflake RAW
dbt build                         # run all models + tests
dbt build --select tag:marts      # marts only
dbt build --select tag:entries    # all entry-related models
...
```
ingestion/              # Python: Supabase to Snowflake (ingest, seed_data, db)
